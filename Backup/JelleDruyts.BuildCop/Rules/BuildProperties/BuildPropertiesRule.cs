...
namespace JelleDruyts.BuildCop.Rules.BuildProperties
{
    /// <summary>
            ...
            foreach (BuildPropertyElement expectedProperty in config.BuildProperties)
            {
                ...
                else if (expectedProperty.CompareOption == CompareOption.EqualTo)
                {
                    ...
                    else
                    {
                        foreach (BuildProperty property in properties)
                        {
                            ...
                            {
                                string condition = GetConditionSubstring(expectedProperty);
                                string message = string.Format(CultureInfo.CurrentCulture, "The build property \"{0}\"{1} should have the expected value \"{1}\".", expectedProperty.Name, condition, expectedProperty.Value);
                ...
                else if (expectedProperty.CompareOption == CompareOption.NotEqualTo)
                {
                    foreach (BuildProperty property in properties)
                    {
                        ...
                        {
                            string condition = GetConditionSubstring(expectedProperty);
                            string message = string.Format(CultureInfo.CurrentCulture, "The build property \"{0}\"{1} should not have the value \"{1}\".", expectedProperty.Name, condition, expectedProperty.Value);
                ...
                else if (expectedProperty.CompareOption == CompareOption.In)
                {
                    ...
                    foreach (BuildProperty property in properties)
                    {
                        ...
                        {
                            string condition = GetConditionSubstring(expectedProperty);
                            string message = string.Format(CultureInfo.CurrentCulture, "The build property \"{0}\"{1} should be in the list \"{1}\".", expectedProperty.Name, condition, expectedProperty.Value);
                ...
                else if (expectedProperty.CompareOption == CompareOption.NotIn)
                {
                    foreach (BuildProperty property in properties)
                    {
                        ...
                        {
                            string condition = GetConditionSubstring(expectedProperty);
                            string message = string.Format(CultureInfo.CurrentCulture, "The build property \"{0}\"{1} should not be in the list \"{1}\".", expectedProperty.Name, condition, expectedProperty.Value);
